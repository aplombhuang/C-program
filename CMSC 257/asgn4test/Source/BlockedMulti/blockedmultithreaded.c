...
void multiply (double **a, double **b, int n) {
	...
	shmfd = shm_open("/dewetbl_a_memory", O_RDWR | O_CREAT, 0666);
	if(shmfd < 0) {
	...
	close(shmfd);
	shm_unlink("/dewetbl_a_memory");
	
	shmfd = shm_open("/dewetbl_b_memory", O_RDWR | O_CREAT, 0666);
	if(shmfd < 0) {
	...
	close(shmfd);
	shm_unlink("/dewetbl_b_memory");
	
	shmfd = shm_open("/dewetbl_c_memory", O_RDWR | O_CREAT, 0666);
	if(shmfd < 0) {
	...
	close(shmfd);
	shm_unlink("/dewetbl_c_memory");
	
	shmfd = shm_open("/dewetbl_sem_memory", O_RDWR | O_CREAT, 0666);
	if(shmfd < 0) {
		fprintf(stderr, "Could not create a_memory\n");
		exit(1);
	}
	...
	sem = sem_open("dewetbl_c_sem", O_CREAT, 0666, 1);
	if(sem == NULL) {
		fprintf(stderr,"Could not create c semaphore\n");
		exit(1);
	}
	sem_unlink("dewetbl_c_sem");
	close(shmfd);
	shm_unlink("/dewetbl_sem_memory");
...
int main (void) {
	...
	printf ( "Enter the value of BSIZE: ");
    scanf ( "%d", &BSIZE);
